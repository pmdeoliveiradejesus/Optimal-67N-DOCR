...
\begin{abstract}
\textcolor{red}{
	  The problem of coordination of directional overcurrent protections (DOCP) in interconnected power systems has been widely treated in literature. All existing procedures are focused on phase 67P function and network models based upon typical positive sequence models. However, other DOCP functions such as 67N (neutral) should be also coordinated accounting the effects of high-impedance phase to earth faults. The optimal coordination  of the neutral function of DOCP has been completely overlooked in literature. In this paper, we investigate the impact of high-impedance single-phase to earth faults in the  clearing times and selectivity of the 67N function of DOCP whose time dial settings were selected using a standard optimization technique.  
	 The multiphase-multiground (4-wire) network model developed by EPRI for the OpenDSS platform is used to determine the exact fault current distribution across phases, neutrals and the ground path.  As a key contribution, we determine the effect of a range of pickup settings and fault resistances on speed, selectivity and voltage profile. The results show that for a range of pickup settings the optimal operation time decreases as the fault impedance increases until the calculated time dial setting stagnates at the minimum value. Beyond this point, the clearing times reach a minimum and deteriorate as far the fault impedance increases.} 
\end{abstract}
...
\normalsize
  ...
  The solution for the optimal clearing time for faults with $R_F$=0 is OF= 3.6551s. The resulting optimal time dial settings are   $D_1$=0.2711,
    ...
    $D_6$=0.2553. It is worth to note that separation times (given by $\mathbf{B}\cdot \mathbf{D}$) are equal to the prescribed coordination time (0.2s) with two exceptions  in rows 4 and 5 of the coordination matrix: -2.53$D_4$+31.45$D_6$=7.34s and   8.74$D_1$-2.62$D_4$=1.69s.
        ...
        \textcolor{red}{Figure 4(a) shows a histogram with the resulting operation times for 5000 random generated faults. The mean primary operation time calculated with the resulting optimal time dial settings is 3.3511s with standard deviation
 0.0780s. In order to verify the appropriateness of the optimal settings obtained, we analyze if the selectivity criteria stills valid when the faults occur in other locations different to near-end or near-far of each line. To do so, we  determine the separation times (given by $\mathbf{B}\cdot \mathbf{D}$ for 5000 random generated faults.    Results of the selectivity simulation are showed in the histogram shown in Fig. 4(b). Observe that no calculated separation time is below the coordination time interval C =200ms predefined by the protection engineer. A histogram with voltage profile for the faulted and non-faulted phases is depicted in  Fig. 5(a)}. 
...
$D_4$=0.1572,
$D_5$=0.1247
, and $D_6$=0.1484. All separation times (given by $\mathbf{B}\cdot \mathbf{D}$ are equal to the prescribed coordination time (0.2s). \textcolor{red}{As done in case 1 we insert 5000 random faults with the resulting optimal primary operation times for dial settings of case 2. The
 histogram shown in Fig. 4(c) indicates that  the mean operation time calculated is  2.0353s with standard deviation
 ...
 Results of the selectivity simulation are depicted in the histogram shown in Fig. 4(d). In this case we also observe full selectivity. A histogram with voltage profile for the faulted and non-faulted phases is depicted in Fig. 5(b). Note that residual voltages are higher than the ones observed in case 1 (Fig. 5(a)). As a matter of fact, phases $a$ and $b$ show a temporary overvoltage lower than 1.2 for all simulated faults.}

 
\subsection{Verification with non-solid faults}\label{s3}

One aspect that we can verify is what happen with speed and selectivity solutions if we setup the TDS of all relays with the optimal settings reported in case 1, but the actual fault magnitude is different than zero as simulated in case 2 ($R_F$=10 $\Omega$). In this case, after simulate 5000 random locations for faults with $R_F$=10 $\Omega$ and a pickup setting of 50A in all relays we observe average operational times larger than the optimal ones reported in case 2. In this case, optimal TD settings for solid faults yield a non-optimal solution when the actual fault impedance magnitude is different than zero. However, selectivity is not affected since separation times are greater than the coordination interval in all feasible primary-backup pairs. This result must not be considered a general case and further elaboration is required.
...
\noindent\textbf{J.S. Abello}  is
electrical engineer degree from the Universidad de los Andes (2019).
 
\noindent\textbf{O. Archilla} is
electrical engineer degree from the Universidad de los Andes (2019).
 

 
\noindent\textbf{C. Alza}  is
electrical engineer degree from the Universidad de los Andes (2019).
